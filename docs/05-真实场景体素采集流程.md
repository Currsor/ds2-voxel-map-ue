...
![单视图采集：一个透视相机观察 CaptureBounds](./diagrams/capture_entry_single_view.svg)

橙色实线表示当前相机可观测的表面；灰色虚线表示被前景遮挡、无法由该入口直接观测的部分。

#### 半球采集

![半球采集：Fibonacci 上半球透视视图](./diagrams/capture_entry_hemisphere.svg)

蓝色采样点均位于世界上半球，每个相机都朝向 `CaptureBounds` 中心；多视角提高外表面覆盖，但深层遮挡仍可能缺失。

#### XYZ 切片采集

![XYZ 切片采集：沿三个世界轴逐层执行正交采集](./diagrams/capture_entry_xyz_slice.svg)

紫色区域是当前允许写入的薄层。X、Y、Z 三轴依次生成任务；启用双向采集时，每层从轴正反方向各观察一次。

#### 联合世界采集

![联合世界采集：半球任务与 XYZ 切片任务进入统一累加器](./diagrams/capture_entry_combined.svg)

半球任务与切片任务共享同一个 `FCaptureAccumulator`，完成后统一过滤并写入资产，不会先生成两份独立资产再拼接。
...
![第 2 步：从采集范围生成视图任务](./diagrams/capture_step2_view_generation.svg)

该图把视图生成阶段分为三步：先从 `CaptureBounds` 和 `VoxelSize` 建立对齐到 `4×4×4` Block 的体素网格，再按入口生成透视或正交任务，最后统一排入 `FCaptureJob` 队列交给 GPU。蓝色表示透视视图，紫色表示正交切片，橙色表示场景表面。
...
![第 4 步：像素重建与体素坐标转换](./diagrams/capture_step4_pixel_reconstruction.svg)

该图展示单个像素从 `DepthPixels` 与 `ColorPixels` 开始，经过透视或正交反投影、邻域法线与置信度过滤，再按 `World X/Z/Y → Voxel X/Y/Z` 轴映射量化为体素坐标，最终生成单视图累加使用的 `LinearIndex + Color + Confidence`。
...
![第 6 步：体素清理与拓扑修复](./diagrams/capture_step6_voxel_cleanup.svg)
...
![第 7 步：稀疏数据构建与资产输出](./diagrams/capture_step7_sparse_asset_output.svg)
